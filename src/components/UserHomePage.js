...
class UserHomePage extends Component {
  constructor(props) {
    ...
    axios.get('http://localhost:5000/api/user/' + this.id).then(res => {
      ...
      this.setState({ name : user.name, email : user.email ,phoneNumber : user.phoneNumber, address: user.address, expertise: user.expertise, comments_enable: user.comments_enable, receive_newsletter: user.receive_newsletter, receive_advice: user.receive_advice});
    });
  }
  ...
  render() {
    ...
    var expertiseChange = this.onExpertiseChange.bind(this);
